...
STATIC_ASSERT(sizeof(RW_mutex) == sizeof(RW_mutex_t),
              mutex_header_imp_same_size);
...
// private
#define CHECK_INIT(MUTEX)                                                      \
  if (!MUTEX || !MUTEX->data_ptr) {                                            \
    return EXIT_FAILURE;                                                       \
  }
...
static int common_lock_data_cmp(struct common_lock *lock, void *buffer,
                                uint8_t buffer_size) {
  uint8_t res = -1;
